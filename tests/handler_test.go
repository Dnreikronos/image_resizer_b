...
func TestResizeImage(t *testing.T) {
	...
	t.Run("Successful Image Resize", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/resize?id=%s&width=50&height=50", image.ID), nil)
		w := httptest.NewRecorder()

		r.ServeHTTP(w, req)

		assert.Equal(t, http.StatusOK, w.Code)
		assert.Contains(t, w.Body.String(), "Image resized")
	})

	t.Run("Invalid Width Parameter", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/resize?id=%s&width=-1&height=50", image.ID), nil)
		w := httptest.NewRecorder()
	...
	t.Run("Invalid Height Parameter", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/resize?id=%s&width=50&height=0", image.ID), nil)
		w := httptest.NewRecorder()

		r.ServeHTTP(w, req)

		assert.Equal(t, http.StatusBadRequest, w.Code)
		assert.Contains(t, w.Body.String(), "Invalid height")
	})

	t.Run("Image Not Found", func(t *testing.T) {
		randomID := uuid.New()
		req := httptest.NewRequest(http.MethodPut, fmt.Sprintf("/resize?id=%s&width=50&height=50", randomID), nil)
		w := httptest.NewRecorder()
